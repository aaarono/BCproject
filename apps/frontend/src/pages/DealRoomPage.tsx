...
export function DealRoomPage() {
  ...
  const [buyerConversationId, setBuyerConversationId] = useState<string | null>(null);
  ...
  async function loadBuyerConversation(dealData: Deal) {
    const res = await http.post(`/conversations`, { listingId: dealData.listingId });
  ...
  if (err || !deal) return <div className="p-6 text-red-600">{err ?? "Not found"}</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {isSeller && deal.status === "FUNDED" && (
              <button
                disabled={busy}
                className="bg-black text-white rounded px-4 py-2 disabled:opacity-50"
                onClick={() => act(() => http.post(`/deals/${deal.id}/delivered`))}
              >
            ...
            {isBuyer && deal.status === "DELIVERED" && (
              <button
                disabled={busy}
                className="bg-black text-white rounded px-4 py-2 disabled:opacity-50"
                onClick={() => act(() => http.post(`/deals/${deal.id}/complete`))}
              >
            ...
            {isSeller && (deal.status === "INITIATED" || deal.status === "FUNDED") && (
              <button
                ...
                onClick={() => act(() => http.post(`/deals/${deal.id}/cancel`))}
              >
        ...
        {isBuyer && deal.status === "COMPLETED" && <ReviewSection dealId={deal.id} />}
      </div>
